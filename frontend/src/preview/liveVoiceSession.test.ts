...
import { LiveVoiceSession, type SocketLike } from "./liveVoiceSession";
import type { MicCapture } from "./audioCapture";
import type { PlaybackQueue } from "./audioPlayback";

/** A fake WebSocket good enough to drive LiveVoiceSession's protocol logic without
 * a browser: readyState + send/close tracking, and manual open/message/close. */
...
function setup() {
  ...
  const session = new LiveVoiceSession("agent-1", callbacks, {
    createSocket,
    createMic,
    createPlayback,
    wsUrl: (id) => `ws://test/${id}`,
...
describe("LiveVoiceSession", () => {
  it("acquires the mic, opens the socket at the (unchanged) route, sends 'start'", async () => {
  ...
  it("dispatches transcript, disclosure, outcome, and error same as Phase 3", async () => {
    const { session, socket, callbacks } = setup();
    await session.start();
    socket.simulateOpen();
  ...
  it("dispatches the new 'tool' event", async () => {
    const { session, socket, callbacks } = setup();
    await session.start();
    socket.simulateOpen();

    socket.simulateMessage(
      JSON.stringify({ type: "tool", name: "calendar", timing: "in_call" }),
    );
    expect(callbacks.onTool).toHaveBeenCalledWith("calendar", "in_call");
  });

  it("dispatches 'moderation' and flushes playback on a BLOCK verdict only", async () => {
    const { session, socket, callbacks, getFlushCount } = setup();
    await session.start();
    socket.simulateOpen();

    socket.simulateMessage(JSON.stringify({ type: "moderation", verdict: "flag" }));
    expect(callbacks.onModeration).toHaveBeenCalledWith("flag");
    expect(getFlushCount()).toBe(0);

    socket.simulateMessage(JSON.stringify({ type: "moderation", verdict: "block" }));
    expect(callbacks.onModeration).toHaveBeenCalledWith("block");
    expect(getFlushCount()).toBe(1);
  });

  it("'cut_playback' flushes audio immediately without a moderation callback", async () => {
    const { session, socket, callbacks, getFlushCount } = setup();
    await session.start();
    socket.simulateOpen();

    socket.simulateMessage(JSON.stringify({ type: "cut_playback" }));
    expect(getFlushCount()).toBe(1);
    expect(callbacks.onModeration).not.toHaveBeenCalled();
  });

  it("pushes binary frames to the playback queue and reports 'agent' speaking", async () => {
    const { session, socket, pushed, callbacks } = setup();
    await session.start();
    socket.simulateOpen();
